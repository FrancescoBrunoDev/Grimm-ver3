...
new Nearby(iconHeartButton, {
  onProgress: (distance) => {
    ...
    tweenHeart.timeScale(
      Math.min(Math.max(time, heartbeatInterval.from), heartbeatInterval.to)
    );
    if (
      ...
      (distance > distanceThreshold.max || distance < distanceThreshold.min) &&
      stateHeart !== "paused"
    ) {
      tweenHeart.pause();
      stateHeart = "paused";
...
new Nearby(link1, {
  onProgress: (distance) => {
    ...
    TweenMax.to(tooltip1, 0.5, {
      ease: "Expo.easeOut",
      scaleX: `${Math.max(
        Math.min(scale, linkLineInterval.to),
        linkLineInterval.from
      )}`,
    });
  },
...
new Nearby(link2, {
  onProgress: (distance) => {
    ...
    tweenWave.timeScale(
      Math.min(Math.max(time, waveInterval.from), waveInterval.to)
    );
